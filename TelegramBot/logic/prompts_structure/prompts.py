...
2. **primary_entity / secondary_entity**:
   ...
   - **name**: Приводи к именительному падежу (Байкале -> Байкал, малом море -> Малое море). НЕ ставь никакие дополнительные символы, например ударения (ско́па -> скопа)
   - **type**: Строго один из: Biological, GeoPlace, Infrastructure, Unknown.
      * Используй `Unknown`, ТОЛЬКО если объект явно не относится к флоре, фауне или достопримечательностям (Infrastructure) Байкальского региона (например: "биткоин", "машина", "биба", "президент США").
   - **category/subcategory**: Заполняй СТРОГО по спискам ниже.
   
3. **category / subcategory**:
{types}

4. **attributes**: Извлекай сезон (season), место обитания (habitat), состояние (цветение) (flower_and_fruit_info).
 - season (сезон), может быть в запросе написано по-разному (наприме, "осенний") приводи такое всегда к "Зима", "Осень", "Лето", "Весна".
 - habitat (место) 
 - fruits_present (признак плода у объекта, например шишка). Всегда приводи к нормальной форме, например "Шишки" -> "Шишка".
 - flowering (признак цветения). Например слова "цветущий", "в цветении". В таком случае просто поставь "flowering": "Да".
             
5. **Сложные типы для определения**
{flora}
